...
// Flag untuk mengetahui apakah input nama aktif
let isNameInputActive = true; // Set default ke true, karena nama harus diinput dulu

// Tombol game start dan pengaturan input
const gameStartBtn = document.getElementById("gameStartBtn");
const gameStartDiv = document.getElementById("gameStartDiv");

// Event listener untuk menangani fokus input nama
document.getElementById('playerName').addEventListener('focus', () => {
  isNameInputActive = true;  // Ketika input nama fokus
});

// Event listener untuk menangani kehilangan fokus input nama
document.getElementById('playerName').addEventListener('blur', () => {
  isNameInputActive = false;  // Ketika input nama tidak fokus
});
...
gameStartBtn.addEventListener("click", () => {
  ...
  if (!playerName) {
    ...
    // Menunda perubahan status isNameInputActive
    setTimeout(() => {
      isNameInputActive = false;
      console.log("isNameInputActive set to false");
      game.scene.resume("scene-game");
    }, 100); // Delay sedikit untuk memastikan semua perubahan selesai sebelum melanjutkan
  }
});
...
class GameScene extends Phaser.Scene {
  ...
  create(){
    ...
    document.getElementById('playerName').addEventListener('focus', () => {
      isNameInputActive = true;  // Ketika input nama fokus
      console.log("Input nama aktif"); // Debugging
    });

    document.getElementById('playerName').addEventListener('blur', () => {
      isNameInputActive = false;  // Ketika input nama kehilangan fokus
      console.log("Input nama tidak aktif"); // Debugging
    });
    ...
    document.getElementById("restartGameBtn").addEventListener("click", () => {
      location.reload(); // Reload halaman untuk kembali ke awal
    });    
  }
  ...
  updateStats() {
    ...
    // Cek jika ada progress bar yang habis
    if (this.hunger === 0 || this.energy === 0 || this.hygiene === 0 || this.happiness === 0) {
        this.gameOver();
    }
  }
  ...
  update(){
    // Cek jika input nama sedang aktif. Jika aktif, kita tidak biarkan WASD menggerakkan karakter.
    console.log(`isNameInputActive: ${isNameInputActive}`);
    if (isNameInputActive) {
      return; // Tidak ada pergerakan jika input nama aktif
    }
